...
class ConcreteTexture extends exports.Texture
    ...
    bind: (unit=0) ->
        #FIXME the caching somehow screws up
        ###
...
exports.Texture2D = class Texture2D extends ConcreteTexture
    ...
    loadImage: (url, flipped=false) ->
        image = new Image()
        image.onload = =>
            @dataImage image, flipped
        image.src = url
        return @

    dataImage: (data, flipped=false) ->
        @bind()
    ...
    dataSized: (data, width, height, flipped=false) ->
