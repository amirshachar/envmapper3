...
exports = class WebGLFramework
    constructor: (params={}) ->
        ...
        @gl = @getContext 'webgl', params #~80ms
        ...
        @textureFloat = textureFloat(@gl) #~80ms

        float32 = @textureFloat.float32
        float16 = @textureFloat.float16
        if float32.renderable and float32.linear
            @usableFloat = float32.type
        else if float16.renderable and float16.linear
            @usableFloat = float16.type
    ...
    depthbuffer: (params) ->
        new framebuffer.Depthbuffer(@, params)
    ...
    vec4: (x, y, z, w) -> new vector.Vec4(x,y,z,w)
